...
class Mascaret():
    ...
    def get_hydraulic_variable(self, ndID, lvl):
        """
        Get hydraulic variables dependent on water level
        @param ndID(int) computational node index
        @param lvl(float) water level
        @return b1, b2, bs(float) width of \
        main channel, flood plain, and storage
        @return p1, p2(float) wetted perimeter of main channel and flood plain
        @return s1, s2(float) flow area of main channel and flood plain
        @return rh1, rh2(float) hydraulic radius \
        of main channel and flood plain
        @return cnvc(float) flow conveyance
        @return beta(float) compound channel coefficient
        """

        id_masc_c = (ctypes.c_int * 1)(self.id_masc)
        ndID_c = (ctypes.c_int * 1)(ndID + 1)
        lvl_c = (ctypes.c_double * 1)(lvl)
        iprint_c = (ctypes.c_int * 1)(self.iprint)
        ...
        rh1 = ctypes.c_double()
        rh2 = ctypes.c_double()
        cnvc = ctypes.c_double()
        beta = ctypes.c_double()

        self.logger.debug(
        'Getting hydraulic variables of computational node {}...'
        .format(ndID_c))
        self.error = self.libmascaret.C_GET_HYDRAULIC_VARIABLE(
        id_masc_c, ndID_c, lvl_c, iprint_c, 
        ctypes.byref(b1), ctypes.byref(b2), ctypes.byref(bs), 
        ctypes.byref(p1), ctypes.byref(p2), 
        ctypes.byref(s1), ctypes.byref(s2), 
        ctypes.byref(rh1), ctypes.byref(rh2), 
        ctypes.byref(cnvc), ctypes.byref(beta))

        return b1.value, b2.value, bs.value, \
        p1.value, p2.value, \
        s1.value, s2.value, \
        rh1.value, rh2.value, \
        cnvc.value, beta.value
