...
class MascaretStudy():
    ...
    def import_model_from_dict(self, settings_dict, xcas=None):
        ...
        for key_val in self.settings['files'].items():
            ...
            if not isinstance(key_val[1], list):
                ...
                if file_types[-1] not in ['res', 'listing', 'res_casier', 
                'listing_casier', 'res_liaison', 'listing_liaison', 
                'tracer_res', 'tracer_listing']:
    ...
    def store_xcoord(self):
        ...
        self.xcoord = []
        for i in range(imax):
            self.xcoord.append(float(Decimal(str(
    ...
    def save_lig_restart(self, out_file='RestartLigneEau.lig', k_s=None):
        ...
        with open(out_file, 'w') as lig:
            ...
            np.savetxt(lig,
                       ...
                       fmt=(nbcol * '%15.3f'), header='X', comments=' ')
            if nbmod != 0:
                np.savetxt(lig,
            ...
            np.savetxt(lig,
                       np.asarray(self.global_elevation[0:nbfmt]).reshape(
                            nblin, nbcol),
                       fmt=(nbcol * '%15.3f'), header='Z', comments=' ')
            if nbmod != 0:
                np.savetxt(lig,
            ...
            np.savetxt(lig,
                       ...
                       reshape(nblin, nbcol),
                       fmt=(nbcol * '%15.3f'), header='Q', comments=' ')
            if nbmod != 0:
                np.savetxt(lig,
            ...
            if k_s is not None:
                ...
                np.savetxt(lig,
                           ...
                           fmt=(nbcol * '%15.3f'), header='CF1', comments=' ')
                if nbmod != 0:
                    np.savetxt(lig,
                ...
                np.savetxt(lig,
                           np.asarray(k_s[0:nbfmt]).reshape(nblin, nbcol),
                           fmt=(nbcol * '%15.3f'), header='CF2', comments=' ')
                if nbmod != 0:
                    np.savetxt(lig,
                               np.asarray(k_s[nbfmt:]).reshape(1, nbmod),
                               fmt=(nbmod * '%15.3f'))
