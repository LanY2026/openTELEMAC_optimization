    ...
    def convert_xcas_telcase(self, lang='en'):
        ...
        for elem in self.root.iter():
            if list(elem) == []:
                ...
                if lang == 'fr':
                    keyword = self.dico.gb2fr[keyword]
    ...
    def write_xcas_file(self, xcas_filename=None):
        ...
        if self.case != None:
            ...
            for key in self.case.values.keys():
                ...
                """
                In self.dico.data, values of keys with digit are incomplete, 
                so the keys with "x" are used here.
                """
                if any(map(str.isdigit, key)):
                    key_dgts = re.findall(r'\d+', key)
                    key_x = key
                    for dgt in key_dgts: key_x = key_x.replace(dgt, 'x')
                    dico_data_key = self.dico.data[key_x]
                else:
                    dico_data_key = self.dico.data[key]

                """
                self.case.values does not have keys with "x", 
                so the keys with digit are used.
                """
                if "ALLOCATED" in dico_data_key:
                    key_alloc = dico_data_key['ALLOCATED']
                    if " x" in key_alloc:
                        key_alloc_case = key_alloc.replace(
                        ' x', (' ' + key_dgts[-1]))
                    else:
                        key_alloc_case = key_alloc
                if "ALLOCATED2" in dico_data_key:
                    key_alloc2 = dico_data_key['ALLOCATED2']
                    if " x" in key_alloc2:
                        key_alloc2_case = key_alloc2.replace(
                        ' x', (' ' + key_dgts[-1]))
                    else:
                        key_alloc2_case = key_alloc2

                if 'ALLOCATED' in dico_data_key and \
                        'ALLOCATED2' in dico_data_key:
                    values = []
                    if isinstance(self.case.values[key_alloc2_case], list):
                        for dim in self.case.values[key_alloc2_case]:
                            list_temp = []
                            for i in range(dim):
                    ...
                    end_path = dico_data_key['PATHNODE'].replace(' ', '').\
                    replace(self.dico.data[key_alloc]['ALLOCATE']
                    .replace(' ', ''), '')
                    if end_path == '':
                        end_path = \
                    ...
                    """
                    For keys with digit, values are given with separate keys 
                    rather than a single key, for multiple items. 
                    So values of items other than first need to be added into 
                    "allocated_dict", and first value should be tree-ed alone.
                    """
                    if any(map(str.isdigit, key)):
                        if int(key_dgts[-1]) >= 2:
                            num_key_alloc = key_alloc + ' ' + key_dgts[-1]
                            if num_key_alloc not in allocated_dict:
                                allocated_dict[num_key_alloc] = OrderedDict()
                                allocated_dict[num_key_alloc]['item'] = \
                                self.dico.data[key_alloc]['ALLOCATE']\
                ...
                elif 'ALLOCATED' in dico_data_key:
                    ...
                    if any(map(str.isdigit, key)):
                        if int(key_dgts[-1]) >= 2:
                            ...
                            if num_key_alloc not in allocated_dict:
                                ...
                                self.dico.data[key_alloc]['ALLOCATE']\
                                .replace(' ', '')
                            allocated_dict[num_key_alloc][end_path] = value[0]
                        else:
                            value = value[0]
                    else:
                        value = value[0]
                ...
                """
                For keys with digit, 
                values of items other than first should not be tree-ed.
                """
                if (not any(map(str.isdigit, key))) or \
                (any(map(str.isdigit, key)) and int(key_dgts[-1]) == 1):
                    if isinstance(value, list):
                        val_temp = ''
                        for val in value:
                            val_temp += str(val) + ' '
                    ...
                    pathnode = dico_data_key['PATHNODE'].replace(" ", "")
