...
void get_hydraulic_variable_(int *, \
                             int *, int *, double *, \
                             int *, \
...
//------------------------------------------------------------------------------
/* 
   Compute hydraulic variables, dependent on water level but not flowrate.
   Inputs:
          mdl_id:     index of model instance
          nd_id:      index of model computational node
          z:          water level
          iprint:     flag value for model listing file
   Outputs:
          b1, b2, bs: width of main channel, flood plain, and storage
          p1, p2:     wetted perimeter of main channel and flood plain
          s1, s2:     flow area of main channel and flood plain
          rh1, rh2:   hydraulic radius of main channel and flood plain
          cnvc:       flow conveyance
          beta:       compound channel coefficient
*/
//------------------------------------------------------------------------------
int C_GET_HYDRAULIC_VARIABLE(int *mdl_id, int *nd_id, double *z, \
                             int *iprint, \
                             double *b1, double *b2, double *bs, \
                             double *p1, double *p2, \
                             double *s1, double *s2, \
                             double *rh1, double *rh2, \
                             double *cnvc, double *beta)
{
   int error_rtn;

   get_hydraulic_variable_(&error_rtn, \
                           mdl_id, nd_id, z, \
                           iprint, \
                           b1, b2, bs, \
                           p1, p2, \
