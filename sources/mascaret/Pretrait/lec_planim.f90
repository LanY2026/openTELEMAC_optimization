...
! *********************************************************************
   ...
   IF (ITABDEB(1) .NE. 1) THEN
      ERREUR%NUMERO = 805
      ERREUR%FT = ERR_805
      ERREUR%FT_C = ERR_805C
      CALL TRAITER_ERREUR(ERREUR, 'NUMERO DU PREMIER PROFIL')
      RETURN
   ENDIF
   ...
   do izone = 1 , nb_zone_planim
      ...
      if( proffin_zone_planim .LT. profdeb_zone_planim ) then
         Erreur%Numero = 314
         Erreur%ft     = err_314
      ...
      !----------------------------------------
      if(profdeb_zone_planim /= proffin_zone_planim_prec + 1) then
         Erreur%Numero = 375
         Erreur%ft     = err_375
